...
def type_hge_pwd(driver, hge_key):
    pwdElem = wait_for_element_by_name(driver, "password")
...
def get_headers(driver):
    ...
    i = 0
    while i < 100:
        (key, value) = get_header_at_index(driver, i)
        if not key:
            return headers
        elif key.lower() not in to_discard_headers:
            headers[key] = value
        i = i + 1

def get_header_at_index(driver, index):
    (key_form, val_form) = get_header_forms_at_index(driver, index)
    if not key_form:
        return (None, None)
...
def get_header_forms_at_index(driver, index):
    ...
    if not key_forms:
        ...
        return (key_forms[0], value_form)

def get_last_header_index(driver):
    i=0
    while i<100:
        (hdr_key_form,_) = get_header_forms_at_index(driver, i)
        if not hdr_key_form:
            return i-1
        i=i+1
...
def type_headers(driver, headers):
    ...
    for (key, value) in headers.items():
        (key_form, value_form) = get_header_forms_at_index(driver, i)
...
def prettify(driver):
    driver.find_element_by_xpath(
        "//*[contains(@title, 'Prettify Query (Shift-Ctrl-P)')]").click()
...
def apply_explorer_conf(driver, conf, max_wait=600):
    ...
    try:
        variables = json.loads(variables, object_pairs_hook=ruamel.yaml.comments.CommentedMap)
        conf['query']['variables'] = variables
    except Exception:
        pass
    respTextArea = get_response_textarea(driver)
    response = get_text(respTextArea)
    conf['response'] = json.loads(response, object_pairs_hook=ruamel.yaml.comments.CommentedMap)
...
def get_all_metadata_queries(driver, url):
    ...
    for request in driver.requests:
        if request.path == url + '/v1/query':
            try:
                body = json.loads(request.body, object_pairs_hook=ruamel.yaml.comments.CommentedMap)
