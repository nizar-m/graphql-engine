...
def reload_remote(hge_ctx, name):
    return hge_ctx.admin_v1q(mk_reload_remote_q(name))

def get_remote_schemas(hge_ctx):
    with hge_ctx.engine.connect() as conn:
        return [row['name'] for row in conn.execute('select name from hdb_catalog.remote_schemas')]
...
class TestRemoteSchemaBasic:
    ...
    def test_add_schema(self, hge_ctx):
        ...
        remote_schemas = get_remote_schemas(hge_ctx)
        assert 'simple 1' in remote_schemas, remote_schemas
    ...
    def test_remote_subscription(self, hge_ctx):
        hge_ctx.check_query_f(self.dir + '/basic_subscription_not_supported.yaml')

    def test_add_schema_conflicts(self, hge_ctx, remote_gql_url):
        """add 2 remote schemas with same node or types"""
        st_code, resp = add_remote(hge_ctx, 'simple 2', remote_gql_url('/hello-graphql'))
        assert st_code == 400
    ...
    def test_add_remote_schema_with_interfaces(self, hge_ctx, remote_gql_url):
    ...
    def test_add_remote_schema_with_interface_err_empty_fields_list(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_iface_err_empty_fields_list.yaml')

    def test_add_remote_schema_err_unknown_interface(self, hge_ctx):
        """add a remote schema with an object implementing unknown interface"""
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_err_unknown_interface.yaml')
    ...
    def test_add_remote_schema_with_interface_err_missing_field(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_err_missing_field.yaml')
    ...
    def test_add_remote_schema_with_interface_err_wrong_field_type(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_iface_err_wrong_field_type.yaml')
    ...
    def test_add_remote_schema_with_interface_err_missing_arg(self, hge_ctx):
        ...
        interface"""
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_err_missing_arg.yaml')
    ...
    def test_add_remote_schema_with_interface_err_wrong_arg_type(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_iface_err_wrong_arg_type.yaml')
    ...
    def test_add_remote_schema_with_interface_err_extra_non_null_arg(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_iface_err_extra_non_null_arg.yaml')

    def test_add_remote_schema_with_union(self, hge_ctx, remote_gql_url):
    ...
    def test_add_remote_schema_with_union_err_no_member_types(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_union_err_no_member_types.yaml')
    ...
    def test_add_remote_schema_with_union_err_unkown_types(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_union_err_unknown_types.yaml')
    ...
    def test_add_remote_schema_with_union_err_subtype_iface(self, hge_ctx):
        ...
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_union_err_member_type_interface.yaml')

    def test_add_remote_schema_with_union_err_wrapped_type(self, hge_ctx):
        """add a remote schema with error in spec for union"""
        hge_ctx.check_query_f(self.dir + '/add_remote_schema_with_union_err_wrapped_type.yaml')

    def test_bulk_remove_add_remote_schema(self, hge_ctx):
        hge_ctx.check_query_f(self.dir + '/basic_bulk_remove_add.yaml')
...
class TestAddRemoteSchemaTbls:
    ...
    setup_files = dir + '/tbls_setup.yaml'
    ...
    def test_add_schema_conflicts_with_tables(self, hge_ctx, remote_gql_url):
        """add remote schema which conflicts with hasura tables"""
        st_code, resp = add_remote(hge_ctx, 'simple2', remote_gql_url('/hello-graphql'))
        assert st_code == 400, resp
        assert resp['code'] == 'remote-schema-conflicts', resp

    def test_add_second_remote_schema(self, hge_ctx, remote_gql_url):
        """add 2 remote schemas with different node and types"""
        name = 'my remote2'
        st_code, resp = add_remote(hge_ctx, name, remote_gql_url('/country-graphql'))
        assert st_code == 200, resp
        st_code, resp = remove_remote(hge_ctx, name)
        assert st_code == 200, resp

    def test_remote_query(self, hge_ctx):
        hge_ctx.check_query_f(self.dir + '/simple2_query.yaml')

    def test_remote_mutation(self, hge_ctx):
        hge_ctx.check_query_f(self.dir + '/simple2_mutation.yaml')

    def test_add_conflicting_table(self, hge_ctx):
        st_code, resp = hge_ctx.admin_v1q_f(self.dir + '/create_conflicting_table.yaml')
        assert st_code == 400
        assert resp['code'] == 'remote-schema-conflicts'
    ...
    @pytest.mark.usefixtures('schema_for_same_scalars_test')
    def test_add_schema_same_type_containing_same_scalar(self, hge_ctx, remote_gql_url):
        """
        test types get merged when remote schema has type with same name and
        same structure + a same custom scalar
        """
        name = 'person-graphql'
        st_code, resp = add_remote(hge_ctx, name, remote_gql_url('/person-graphql'))
        assert st_code == 200, resp
        st_code, resp = remove_remote(hge_ctx, name)
        assert st_code == 200, resp
...
class TestAddRemoteSchemaCompareRootQueryFields:
    ...
    def test_schema_check_arg_default_values_and_field_and_arg_types(self, hge_ctx, remote):
        with open('queries/graphql_introspection/introspection.yaml') as f:
            query = yaml.safe_load(f)
        ...
        for fldR in remote_root_ty_info['fields']:
            fld_name =  fldR['name']
            fldH = get_fld_by_name(hasura_root_ty_info, fld_name)
            assert fldH != None, 'Field ' + fld_name + ' in the root query type of remote schema is not found in the root query type of Hasura schema'
...
def get_query_root_ty_name(res):
    return res['data']['__schema']['queryType']['name']

def get_types(res):
    return res['data']['__schema']['types']

def get_query_root_info(res):
    root_ty_name = get_query_root_ty_name(res)
    types = get_types(res)
    return get_type_by_name(types, root_ty_name)

def check_introspection_result(res, exp_types, exp_node_names):
    all_types = _map(lambda t: t['name'], get_types(res))
    print(all_types)
    q_root = get_query_root_info(res)
    assert q_root is not None, 'query root type info not found in schema'
    ...
    for ty_name in exp_types:
        if ty_name not in all_types:
            satisfy_ty = False

    for nn in exp_node_names:
...
def get_type_by_name(types, tyName):
    tys_list = _filter(lambda t: t['name'] == tyName, types)
    assert len(tys_list) <= 1, "More than one type with name " + tyName
    return head(tys_list)

def get_fld_by_name(ty, fldName):
    flds_list = _filter(lambda f: f['name'] == fldName, ty['fields'])
    assert len(flds_list) <= 1, "More than one field with name " + fldName
    return head(flds_list)

def get_arg_by_name(fld, argName):
    args_list = _filter(lambda a: a['name'] == argName, fld['args'])
    assert len(args_list) <= 1, "More than one argument with name " +  argName
    return head(args_list)

def compare_args(argH, argR, arg_path):
    assert argR['type'] == argH['type'], yaml.dump({
        'error' : 'Types do not match for argument ' + arg_path,
...
def compare_flds(fldH, fldR):
    ...
    for argR in fldR['args']:
        ...
        argH = get_arg_by_name(fldH, argR['name'])
        assert argH != None, 'Argument ' + arg_path + ' in the remote schema is not found in Hasura schema'
