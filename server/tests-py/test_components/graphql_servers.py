...
class GraphQLServers:
    ...
    def configure_jwt(self):
        ...
        self.jwt_key_file = self.output_dir + '/' + scenario_name(
            self.scenario) + '_jwt_private.key'
        with open(self.jwt_key_file, 'w') as f:
            f.write(get_private_pem(self.jwt_key))
        self.jwt_conf = {'type': 'RS512', 'key': get_public_pem(self.jwt_key)}
    ...
    def ssl_root_dir(self):
        return os.path.abspath(self.output_dir + "/ssl-" + self.conf_hash)
    ...
    def create_custom_ssl_certs_dir(self):
        ...
        self.custom_ca_crts_file = self.custom_ssl_certs_dir + \
            "/custom-ca-certificates.crt"
        shutil.copyfile(self.custom_ssl_certs_dir + "/ca-certificates.crt",
                        self.custom_ca_crts_file)
    ...
    def set_auth_webhook_ssl_crts(self, ca_key, ca_cert):
        (webhook_key,
         webhook_cert) = gen_ca_signed_keys_and_cert(ca_key, ca_cert)
        ssl_dir = self.ssl_root_dir()
    ...
    def auth_webhook_url(self):
        if self.auth_webhook_root_url:
            return self.auth_webhook_root_url + scenario_auth_webhook_path(
                self.scenario)
        else:
            return None
    ...
    def run(self):
        ...
        if self.hge_config.get('withStackExec'):
            self.run_graphql_engines_with_executable(
                stack_exec_args=self.hge_config['withStackExec'].get(
                    'stackExecArgs', []),
                rts_opts=rts_opts)
        elif self.hge_config.get('withExecutable'):
            self.run_graphql_engines_with_executable(
                hge_executable=self.hge_config['withExecutable'],
                rts_opts=rts_opts)
        elif self.hge_config.get('withDocker'):
            self.run_graphql_engines_with_docker(rts_opts=rts_opts)
    ...
    def get_auth_env(self):
        ...
        if self.jwt_key:
            ...
            webhook_mode = scenario_auth(self.scenario)['webhook'].get(
                'mode', 'get').upper()
    ...
    def run_auth_webhook(self):
        ...
        log_file = os.path.abspath(self.output_dir + '/' +
                                   scenario_name(self.scenario) +
                                   '_auth_webhook.log')
        print(Fore.YELLOW, "Running auth webhook on port", port,
              Style.RESET_ALL)
        ...
        tests_info_db.add_reserved_process_port(port,
                                                self.auth_webhook_process.pid,
                                                "auth webhook")
    ...
    def get_hge_env(self, db_url, port, tixFile, evts_webhook_port):
        env = {
            **os.environ,
            **self.default_graphql_env, 'HPCTIXFILE': tixFile,
    ...
    def print_hge_env(self, env):
        for k in env:
            if k.startswith('HASURA_GRAPHQL') or 'WEBHOOK' in k:
                print(Fore.BLUE, k, Fore.YELLOW, '=>', Fore.LIGHTGREEN_EX,
                      env[k], Style.RESET_ALL)

    def get_hge_env_and_args(self, port, db_url, evts_webhook_port):
        self.hge_index += 1
        prefix = self.output_dir + '/grapqh-engine-' + str(self.hge_index) \
            + '-' + auth_type(self.scenario) + '-' + \
            scenario_name(self.scenario) + '-' + self.conf_hash
        log_file = prefix + '.log'
    ...
    def verify_docker_conf(self):
        ...
        if 'image' not in hge_with_docker:
            raise GraphQLServerError(
                "Hasura GraphQL Engine docker image is not defined"
                " within 'withDocker'"
            )
    ...
    def run_graphql_engines_with_docker(self, rts_opts=[]):
        ...
        for (i, (replica, db_url)) in enumerate(hges_conf):
            ...
            if not replica:
                evts_webhook_port = self.get_unused_port_for_pytest(
                    evts_webhook_port, "pytest events webhook")

            rts_opts_args = []
            if rts_opts:
                rts_opts_args = ['+RTS', *rts_opts, '-RTS']
            (hge_env, extra_args,
             log_file) = self.get_hge_env_and_args(port, db_url,
                                                   evts_webhook_port)
            process_args = [
                'graphql-engine', *rts_opts_args, 'serve', *extra_args
            ]
            ...
            print("Running GraphQL Engine docker with image ", Fore.YELLOW,
                  self.docker_image, Style.RESET_ALL, ", and command: ",
                  Fore.YELLOW, *process_args, Style.RESET_ALL)
            cntnr = self.docker_client.containers.run(self.docker_image,
                                                      ...
                                                      network_mode='host',
                                                      volumes=volumes)
            threading.Thread(target=self.collect_docker_logs,
                             args=[cntnr, log_file]).start()
            ...
            if replica:
                ...
                remote_gql_port = self.get_unused_port_for_pytest(
                    remote_gql_port, "pytest remote GraphQL server")
    ...
    def run_graphql_engines_with_executable(self,
                                            hge_executable=None,
                                            stack_exec_args=[],
                                            rts_opts=[]):
        port = 8080
        ...
        for (i, (replica, db_url)) in enumerate(hges_conf):
            ...
            rts_opt_args = []
            if rts_opts:
                rts_opt_args = ['+RTS', *rts_opts, '-RTS']
            if hge_executable:
                process_args = [hge_executable, *rts_opt_args, 'serve']
            else:
                process_args = [
                    'stack', 'exec', *stack_exec_args, 'graphql-engine',
                    '--', *rts_opt_args, 'serve'
                ]
            process_args.extend(extra_args)
            print('Running GraphQL Engine as: ', Fore.YELLOW, *process_args,
                  Style.RESET_ALL)
            if replica:
                # Starting replicas on same DB at the same time might result in
                # query errors and GraphQL engine might exit at initialization.
                # So we wait for the first GraphQL server to be up
                # before starting its replica.
                self.check_if_graphql_servers_are_running(timeout=20)
            try:
                f = open(log_file, 'w')
                ...
                f.close()
                raise
            tests_info_db.add_reserved_process_port(port, p.pid,
                                                    'GraphQL engine')
    ...
    def check_if_graphql_servers_are_running(self, timeout=60):
        if timeout <= 0:
            raise GraphQLServerError(
                "Timeout waiting for graphql processes to start")
        ...
        for u in self.hge_urls:
            try:
                if not is_graphql_server_running(u):
                    ...
                    return self.check_if_graphql_servers_are_running(timeout -
                                                                     1)
    ...
    def collect_docker_logs(self, cntnr, log_file):
        print(Fore.YELLOW,
              "Collecting logs from GraphQL Engine docker container ",
              cntnr.name, Style.RESET_ALL)
    ...
    def stop_docker(self, cntnr):
        print(Fore.YELLOW, "Stopping GraphQL Engine docker container ",
              cntnr.name, Style.RESET_ALL)
        cntnr.stop()
        print(Fore.YELLOW, "Removing GraphQL Engine docker container ",
              cntnr.name, Style.RESET_ALL)
        cntnr.remove()
    ...
    def stop_hge_processes(self):
        for (p, f) in self.hge_processes:
            print(Fore.YELLOW, "Stopping GraphQL engine: pid", p.pid,
                  Style.RESET_ALL)
            p.send_signal(signal.SIGINT)
            p.wait()
            tests_info_db.remove_process_ports(p.pid)
            f.close()
        self.hge_processes = []

    def stop_hge_dockers(self):
        run_concurrently([
            threading.Thread(target=self.stop_docker, args=[cntnr])
            for (cntnr, log_file) in self.hge_containers
        ])
        self.hge_containers = []
